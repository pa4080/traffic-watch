...
**Use 'traffic-watch' manually.** To run the script as root and push it into the background we shall use [`sudo -b`][1] *(Probably `2>/dev/null` is not obligatory, because, I think all, errors are redirected to `/dev/null` by the script itself.)*:
